...
public class WeeklyValidator implements Validator<Weekly> {
    ...
    private void validateClassFields(Weekly weekly) {
        ...
        // if (weekly.isMonday().) PREGUNTAR SI LOS BOOLEANOS DE LAS FECHA HACER VALIDADOR YA QUE SE
        // LE ASIGNA UN VALOR ENTONCES NUNCA VA A SER VACIO
        // || Constraints.Weekly.MONDAY_NULLABLE))

        if (!(this.expresionValidator
                .isPresent(String.format("%1$tY-%1$tm-%1$td", weekly.getStart()))
    ...
    // if (weekly.isMonday().) PREGUNTAR SI LOS BOOLEANOS DE LAS FECHA HACER VALIDADOR YA QUE SE LE
    // ASIGNA UN VALOR ENTONCES NUNCA VA A SER VACIO
    // || Constraints.Weekly.MONDAY_NULLABLE))

    if(!(this.expresionValidator.isPresent(String.format("%1$tY-%1$tm-%1$td",weekly.getStart()))||Constraints.Weekly.START_NULLABLE))throw new ErrorResponse("weekly start is required",HttpStatus.BAD_REQUEST);
