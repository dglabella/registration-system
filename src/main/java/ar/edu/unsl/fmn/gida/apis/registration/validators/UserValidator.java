...
public class UserValidator implements Validator<User> {
    ...
    private void validateClassFields(User user) {
        ...
        if (!(this.expresionValidator.isPresent(user.getUserLastName())
            ...
            throw new ErrorResponse("user last name is required", HttpStatus.BAD_REQUEST);

        if (!(this.expresionValidator.isPresent(user.getDni()) || Constraints.User.DNI_NULLABLE))
        ...
        if (!(this.expresionValidator.isPresent(user.getPassword())
            ...
            throw new ErrorResponse("user password is required", HttpStatus.BAD_REQUEST);
        ...
        if (!(Constraints.User.LAST_NAME_MIN_LENGHT < user.getUserLastName().length()
                && user.getUserLastName().length() < Constraints.User.LAST_NAME_MAX_LENGHT))
            throw new ErrorResponse(
                    "invalid user last name: must contain between "
        ...
        if (!(Constraints.User.ACCOUNT_MIN_LENGHT < user.getAccount().length()
                && user.getAccount().length() < Constraints.User.ACCOUNT_MAX_LENGHT))
            throw new ErrorResponse(
                    "invalid user account: must contain between "
                            + Constraints.User.ACCOUNT_MIN_LENGHT + " and "
                            + Constraints.User.ACCOUNT_MAX_LENGHT + " characters",
                    HttpStatus.BAD_REQUEST);

        if (!(Constraints.User.PASSWORD_MIN_LENGHT < user.getPassword().length()
                && user.getPassword().length() < Constraints.User.PASSWORD_MAX_LENGHT))
            throw new ErrorResponse(
                    "invalid user password: must contain between "
        ...
        if (!this.expresionValidator.composedName(user.getUserName()))
            throw new ErrorResponse("invalid user name: must have only valids chararcters",
                    HttpStatus.BAD_REQUEST);

        if (!this.expresionValidator.composedName(user.getUserLastName()))
            throw new ErrorResponse("invalid user last name: must have only valids chararcters",
                    HttpStatus.BAD_REQUEST);
