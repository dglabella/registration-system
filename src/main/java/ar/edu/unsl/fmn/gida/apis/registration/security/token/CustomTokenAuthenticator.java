...
public class CustomTokenAuthenticator implements TokenAuthenticator {
	...
	public UsernamePasswordAuthenticationToken authenticate(String token) {
		...
		Claims claims;
		try {
			claims = Jwts.parserBuilder().setSigningKey(TokenProperties.KEY.getBytes()).build()
			...
			ArrayList<LinkedHashMap<String, String>> authorities =
					(ArrayList) claims.get(TokenProperties.PRIVILEGES_CLAIM_KEY);
			...
			throw new ErrorResponse(RegistrationSystemApplication.MESSENGER
					.getAuthenticationMessenger().invalidToken(), HttpStatus.FORBIDDEN);
		}
		return ret;
	}
}
